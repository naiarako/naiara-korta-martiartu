...
<div class="caption">
Cross section of the 3D sensitivity kernel (a) with and (b) without including geometrical spreading. (c) Sensitivity kernel in (b) represented as a function of distances indicated in (b) with dashed lines between the positions of the emitter \(E = mc^2\), $$ \mathbf{x}_s $$, receiver $$ \mathbf{x}_r $$, and an arbitrary spatial location $$ \mathbf{x} $$.
</div>
